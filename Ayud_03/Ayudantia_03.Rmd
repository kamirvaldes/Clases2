...
ggplot(data= cat_ocup,mapping = aes(x = cae_general_red, y = V1, fill = cae_general_red))+ geom_bar(stat = 'identity')
```
...
#fill te da el grfico en colores y te dice las agrupaciones, cae general red variables discretas
```
...
ggplot(data = Tabla_19_19,aes(x = cae_general_red,y = V1, fill= cae_general_red))+geom_bar(stat = 'identity') + facet_wrap(~sexo,ncol = 2, scales = 'free_y', as.table = T) 

ggplot(data = Tabla_19_19[mes_central == params$month,], aes(x = cae_general_red, y = V1, fill = cae_general_red)) + 
  geom_bar(stat="identity") +
  facet_wrap(~sexo,ncol = 2,scales = 'free_y',as.table = T) #scale dice que eje se comparte y se deja libre
```
...
ggplot(data = Tabla_19_19,mapping = aes(x = cae_general_red, y = V1, fill= cae_general_red))+ geom_bar(stat =  'identity')+
scale_y_continuous(labels = number_format(scale = 0.0000001, suffix = 'M',big.mark = ".",decimal.mark = ","))+
labs(x= 'categoria ocupacional', y='N°personas', fill=NULL)+
...
var_sexo[,sexo_porcentual:= round((sexo_absoluto/sexo_anterior),4)*100]
```
...
ggplot(data = var_sexo,mapping = aes(x = mes_nombre, y = sexo_porcentual, fill= sexo)) + geom_line(position = position_identity())
